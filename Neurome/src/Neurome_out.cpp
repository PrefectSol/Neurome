...
std::atomic<bool> Neurome_out::m_running(true);

Neurome_out::Neurome_out()
...
void Neurome_out::sigintHandle(int signal)
{
	m_running = false;
}

bool Neurome_out::openProcess()
{
	if (!m_process.getProcess(m_settings.clientName))
	{
		...
		bool *loop = reinterpret_cast<bool*>(&Neurome_out::m_running);
...
std::string Neurome_out::getCfgPath() const
{
...
bool Neurome_out::readUserConfig(UserConfig_t *userConfig) const
{
	if (!userConfig)
	{
		return false;
...
void Neurome_out::toWindowedMode() const
{
	ConfigHandler osuConfig(getCfgPath());
...
float Neurome_out::execute(const torch::Tensor &actions, uint32_t width, uint32_t height)
{
	return 0.0f;
...
Neurome_out::Settings_t::Settings(std::string clientName, bool isAwaitProcess,
...
void Neurome_out::Settings_t::init(std::string settingsPath)
{
...
Neurome_out::ResultCode Neurome_out::Settings_t::merge(ConfigHandler *configHandler)
{
	if (!configHandler)
	{
...
void Neurome_out::Settings_t::print() const
{
...
void Neurome_out::Settings_t::parseStr(ConfigHandler *configHandler, std::string *value, std::string field)
{
...
void Neurome_out::Settings_t::parseUInt(ConfigHandler *configHandler, uint32_t *value, std::string field)
{
	const std::string str = (*configHandler)[field];
...
void Neurome_out::Settings_t::parseFloat(ConfigHandler *configHandler, float *value, std::string field)
{
	const std::string str = (*configHandler)[field];
	if (isFloat(str))
	{
...
void Neurome_out::Settings_t::parseBool(ConfigHandler *configHandler, bool *value, std::string field)
{
...
void Neurome_out::Settings_t::parseKey(ConfigHandler *configHandler, std::string *value, std::string field)
{
